// content.js — injects a small overlay with a cancelable countdown
(() => {
  ...
  function ensureUI() {
    ...
    shadow.getElementById('mtc-cancel').addEventListener('click', () => {
      stop();
      try { chrome.runtime.sendMessage({type:'mtc:cancel'}); } catch {}
      destroy();
    });
  ...
  function start(sec) {
    ...
    if (secondsLeft === 0) {
      stop();
      try { chrome.runtime.sendMessage({type:'mtc:closeMe'}); } catch {}
      return;
    }
